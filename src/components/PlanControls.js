...
export default function TodoControls(props) {
  ...
  const hasCheckedItems = plan && plan.filter(x => x.checked).length > 0;
  ...
  const allTodosAreCompleted = plan.every(item => item.checked === true);
  const someTodosAreCompleted = plan.every(item => item.checked === true);
  return (
    <Layout>
      <TodoInput
        value={inputText}
      ...
      />
      <ControlBar>
        {plan.length > 0 && !allTodosAreCompleted && (
          <SelectAllButton
